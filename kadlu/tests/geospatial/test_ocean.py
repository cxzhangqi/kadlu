...
def test_load_bathymetry_from_a_single_chs_file():
    provider = Ocean(bathy="CHS", south=43, west=-60, north=44, east=-59)
...
def test_query_for_bathymetry_grid_data():
    ...
    provider = Ocean(bathy=(bathy,lat,lon))
    b = provider.bathy()
...
def test_query_for_temp_grid_data():
    ...
    provider = Ocean(temp=(temp,lat,lon,z))
    b = provider.temp()
    assert isinstance(b, tuple)
