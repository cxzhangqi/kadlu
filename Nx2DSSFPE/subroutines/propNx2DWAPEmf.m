function [x,y,z,psifinal,Af] = propNx2DWAPEmf(...
    icase,...
    freq,xs,ys,zs,...
    PEStarter,c0,...
    numstep,ny,nz,...
...
if strcmpi(sub_EnvInput,'default'), envInput = @sub_envInput_Nx2Dmf; 
else
    if strfind(sub_EnvInput,'.m'), sub_EnvInput = sub_EnvInput(1:strfind(sub_EnvInput,'.m')-1); end
    eval(sprintf('envInput = @%s;',sub_EnvInput))
end
if strcmpi(sub_Output,'default'), modelOutput = @sub_output_Nx2Dmf; 
else
...
% --- initialize the grid ---
Ez_y = nan(ny,1,nf);
Ez = nan(length(Ez_z),ny,length(x),nf);
[Y,Z]=meshgrid(y,z);
if isSingle,
    psi = zeros([size(Z) nf],'single');    % allocate memory for the following variables
    fr_half = zeros([size(Z) nf],'single');
    fr_full = zeros([size(Z) nf],'single');
    atten0 = nan([size(Z)],'single');
    U = zeros([size(Z) nf],'single');
    Af=nan(nz/2,length(x),nf,'single');
else
    psi = zeros(size(Z),nf);    % allocate memory for the following variables
    fr_half = zeros([size(Z) nf]);
    fr_full = zeros([size(Z) nf]);
    fr_Nx2D = zeros([size(Z) nf]);
    atten0 = nan([size(Z) nf]);
    U = zeros([size(Z) nf]);
    Af=nan(nz/2,length(x),nf);
end
nfft = 0;
...
ArtificialAbsorpCoeff =  1/log10(exp(1))./pi;     % 20dB to zmax and ymax, Gaussian decayed length: D
...
atten0 = sqrt(-1) * ArtificialAbsorpCoeff * exp(-(abs(Z(:))-max(abs(z))).^2/D.^2);      % the sponge layer on z
atten0 = reshape(atten0,nz,ny);
clear ArtificialAbsorpCoeff ThinknessOfArtificialAbsorpLayer D

if isplot, % plot the sponge layer
    tempfreq=1;
    figure(37); atten_temp=imag(k0(tempfreq)*sqrt(1+atten0))*20*(c0/freq(tempfreq))*log10(exp(1));
...
StartOut = sub_CylindPEStartermf(k0,kz,PEStarter,zs);
psi(:,:,:)=permute(StartOut(:,:,ones(1,ny)),[1 3 2]);

% --- initial Nx2D free propagator
fr_half(:,:,:)=permute(repmat((exp(1i*dx/2*( sqrt(k0sq(ones(1,nz),:)-kz(:,ones(1,nf)).^2) - k0(ones(1,nz),:) ))),[1 1 ny]),[1 3 2]);
fr_full(:,:,:)=permute(repmat((exp(1i*dx*  ( sqrt(k0sq(ones(1,nz),:)-kz(:,ones(1,nf)).^2) - k0(ones(1,nz),:) ))),[1 1 ny]),[1 3 2]);
...
if ~exist('smoothing_length_rho','var')||isempty(smoothing_length_rho),
    smoothing_length_rho = c0./freq(end)/4;
...
dista = 0;
iNextOutput = 1;
Af(:,iNextOutput,:) = modelOutput(icase,psi,ny,nz,nf,dista,ndy_3DSliceout,ndz_3DSliceout,YZSlice_output_folder,isplot);
...
for jj=1:numstep
    ...
    if isnewscreen,
        ...
        %    1/2./(k0.^2)*(d2denin(:,isupdate)./denin(:,isupdate) - 3/2*(ddenin(:,isupdate)./denin(:,isupdate)).^2) ) ) );
         ...
         U(:,isupdate,:) = exp(1i*dx*permute(k0(ones(nz,1),:,ones(ny,1)),[1 3 2]).*(-1 + sqrt( n2in(:,isupdate,ones(nf,1)) + atten0(:,isupdate,ones(nf,1)) + ...
            1/2./(permute(k0(ones(nz,1),:,ones(ny,1)),[1 3 2]).^2).*(d2denin(:,isupdate,ones(nf,1))./denin(:,isupdate,ones(nf,1)) - ...
            3/2*(denin(:,isupdate,ones(nf,1))./denin(:,isupdate,ones(nf,1)).^2) ) ) ) );
    end
...
psifinal = ifft(psi).*exp(1i*permute(k0(ones(nz,1),:,ones(ny,1)),[1 3 2])*dista).*sqrt(denin(:,:,ones(nf,1)))/sqrt(dista);   nfft = nfft+1; 
psifinal = fftshift(psifinal(1:nz/2,:),2);
z=z(1:nz/2);
y=fftshift(y);
fprintf('----- Finished in %.2f sec for a %.2f m run with %d ffts. -----\n\n',toc,dista,nfft)
