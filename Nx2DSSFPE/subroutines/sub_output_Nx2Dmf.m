function [Af] = sub_output_Nx2Dmf(icase,psi,ny,nz,nf,dista,ndy_3DSliceout,ndz_3DSliceout,YZSlice_output_folder,isplot)

% use PERSISTENT variables declared before
global Y Z k0 kz nfft
global wd_mask n2in denin
global Ez Ez_z Ez_y test
...
if dista ~= 0,
    ...
    % end
    %Ez(:,:,iout_Ez,:) = (sum(permute(Ez_ifft_kernel(ones(ny,1),:,ones(nf,1)),[2 1 3]).*psi,1)).*exp(1i*permute(k0(round(Ez_z/(Z(2)-Z(1)))+1,:,ones(ny,1)),[1 3 2])*dista)/sqrt(dista).*sqrt(denin(round(Ez_z/(Z(2)-Z(1)))+1,:,ones(nf,1))); 
    
    Ez(:,:,iout_Ez,:) = sum(permute(Ez_ifft_kernel(ones(ny,1),:,:,ones(nf,1)),[2 1 3 4]).*permute(psi(:,:,:,ones(1,1)),[1 2 4 3]),1).*exp(1i*shiftdim(k0(round(Ez_z/(Z(2)-Z(1)))+1,:,:,ones(ny,1)),2)*dista)/sqrt(dista).*sqrt(denin(round(Ez_z/(Z(2)-Z(1)))+1,:,:,ones(nf,1)));
    
    psi = ifft(psi).*exp(1i*permute(k0(ones(nz,1),:,ones(ny,1)),[1 3 2])*dista)/sqrt(dista).*sqrt(denin(:,:,ones(nf,1)));   nfft = nfft+1; 
else
    psi = ifft(psi);   nfft = nfft+1; 
end

Af=psi(1:nzhalf,yso,:); %sound intensity on the vertical x-z plane crossing the source at all frequencies
