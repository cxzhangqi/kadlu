function [isnewnsq,isnew_env] = sub_envInput_Nx2Dmf(freq,c0,dista,dx,xs,ys,smoothing_length_rho,smoothing_length_ssp)
...
if isempty(wd_new),   % allocate variables for faster env updates
    ...
    Z_sub_wd = nan(size(Z));   wd_mask = nan(size(Z));
    H_rho = nan(size(Z));   ddenin = nan(size(Z));   d2denin = nan(size(Z));
    denin = nan(size(Z));
    wd_x_next = 0;  NSQ_x_next = 0;
    H_c = nan(size(Z));
    n2w_new = zeros(size(Z));  n2w = zeros(size(Z));
    n2in = nan(size(Z));
...
if (dista==dx/2),
    ...
    cb = cb - 1i*cbi;
    n2b = (c0./cb).^2;
    %%%%%%%%%%%%%%%%PATCH FOR NOW - THERE SEEMS TO BE AN ERROR ABOVE IN
    %%%%%%%%%%%%%%%%CALC BOTTOM SS
    n2b=mean(n2b);
    cb=mean(cb);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
